...
interface FeedComposerFloatingButtonProps {
  sectionStatus: "feeds" | "comments";
  targetFeed: Feed | null;
}

export default function FeedComposerFloatingButton({
  sectionStatus,
  targetFeed,
}: FeedComposerFloatingButtonProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const isAnyModalOpen = useFloatingModalStore(
    (state) => state.isFloatingModalOpen,
  );
