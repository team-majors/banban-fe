...
import { useDraft } from "../feed/useDraft";
import { usePostContent } from "../../api/usePostContent";
...
export const useFloatingInputModal = ({
  ...
  const { getDraft, saveDraft, clearDraft, restoreDraft } =
    useDraft(actionType);
  ...
  const [pendingDraftContent, setPendingDraftContent] = useState<string | null>(
    null,
  );
